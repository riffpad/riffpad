...
// Auth command tests (OAuth device flow retry, host-credential sync),
// moved from cmd/riffpad during the #282 split, plus authCmd/logoutCmd
...
func TestLogoutCmdClearsToken(t *testing.T) {
	dir := t.TempDir()
	if err := config.Update(dir, func(c *config.Config) {
		c.RelayToken = "tok"
		c.RelayUser = "alice"
