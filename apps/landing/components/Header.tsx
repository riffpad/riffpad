      ...
      <div className="mx-auto flex h-14 max-w-frame items-center justify-between px-4 sm:px-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <a
            ...
            className="btn btn-primary ml-1 hidden h-10 px-4 sm:inline-flex"
          >
            {t.nav.cta}
          </a>
          <button
            type="button"
            onClick={() => setOpen((v) => !v)}
            className="flex h-11 cursor-pointer items-center gap-1 rounded-sm px-2 text-xs font-bold text-body transition-colors hover:text-ink md:hidden"
            aria-expanded={open}
