...
- workspace 可删除（软删除先不做，直接删 workspace + messages + 沙箱目录）。
- 列表页管理能力：搜索（名称/代号/备注）、排序（最近活跃/创建时间/名称）、状态筛选（全部/活跃/已归档）、网格/列表视图切换、重命名、备注、置顶、复制链接、归档/恢复、批量选择与批量删除、无限滚动分页。
- 标签（Tags）体系暂不做，状态筛选已覆盖主要分类诉求。
