...
| 里程碑 | 目标 | 预计周期 | 状态 |
|---|---|---|---|
| M0 | 验证核心闭环：Claude Code 等待审批 → 手机批准 → agent 继续 | 2 周 | `[ ]` |
| M1 | MVP：daemon + relay + PWA，E2EE，Claude Code / Codex | 6 周 | `[ ]` |
| M2 | 国内落地：安卓原生壳 + 厂商推送 + 国内中继，国产 CLI | 10 周 | `[ ]` |
| M3 | 会话分享、tmux 兜底、自部署中继、团队版 | 后续 | `[ ]` |
...
**验收**：跨网络（手机蜂窝网）可用；推送 < 5s；10 个种子用户连续使用一周。
...
| M1.15 | 种子用户招募与反馈收集 | `[ ]` | ≥ 10 个用户；7 日留存数据可看 | — |
