...
**验收**：跨网络（手机蜂窝网）可用；10 个种子用户持续使用（推送由原生 app 通道在 M2.2 提供）。
...
| M1.13 | 推送（Web Push VAPID） | `[!]` | **由原生 app 推送替代（M2.2）**：Web Push 需要服务端→浏览器通道，且无法覆盖国内厂商推送；原生壳（M1.14/M2.1）配 APNs/FCM/厂商通道是正路，跳过 Web Push 避免做两遍 | — |
...
**M1 出口条件**：M1.1–M1.16 全部完成（M1.13 以原生推送替代，见上）；种子用户可自助安装 daemon（`curl -fsSL https://riffpad.ai/install.sh | sh`）并完成全流程。
