...
| M1.19 | client-beta UI v2：按 design-system 重做视觉（Console-Mobile：Geist Mono / GitHub green / hairline 卡片）+ 入场/状态动画 + Web 多语种（zh/en，自动检测 + 手动切换） | `[~]` | app.riffpad.ai 与本地 8787 同一产物；深/浅色跟随系统；动画尊重 reduced-motion；中英切换即时生效 | #106 |
| M1.16 | 种子用户招募与反馈收集 | `[ ]` | ≥ 10 个用户；留存数据可看 | — |

**M1 出口条件**：M1.1–M1.19 全部完成（M1.13 由原生推送替代、M1.14 暂缓，见上）；种子用户可自助安装 daemon（`curl -fsSL https://riffpad.ai/install.sh | sh`）并完成 Web 全流程。
