...
1. 多 Project 的本质是"按 owner 查询 workspace 列表"——这是一个数据库查询，内存结构做不了。
2. 多 Project 还要求切回旧项目时能恢复聊天记录——消息必须持久化。
...
## 3. 概念澄清：Project vs Workspace

建议**不做两个实体**。当前代码里的 `Workspace` 就是用户视角的 "Project"：

- 后端、API、数据库继续叫 `workspace`（与 TSD 一致）。
- 前端 UI 可以叫"项目 / Projects"。
- 未来如确需"一个 Project 包含多个环境/分支"的概念，再加 `Project` 聚合层，现在不预先抽象。

## 4. 分阶段方案

### Phase 1：Postgres 持久化 + 项目列表（MVP）

目标：重启 API 不丢项目；用户能看到自己的项目列表，点进去能继续之前的对话。
...
- 新增 `GET /api/v1/workspaces`（按 owner 分页列表）。
- WebSocket 连接建立时：从 DB 加载消息历史并作为 `history` 事件发给前端（或新增 `GET /workspaces/:id/messages`，前端在 WS 前先拉一次，更省 WS 逻辑）。
- 前端项目列表页：卡片展示 slug / name / lastActiveAt，点击进入工作区视图；提供"新建项目"。
...
- 打开已休眠项目时：从 MinIO 恢复快照 → 重建沙箱 → status 转 WARM。
- 之后才能真正说"项目可长期保存"。

### Phase 3：Auth 与 Redis

- 接入 Supabase Auth，ownerID 从 JWT 解析，替换 `anonymous`。
- Redis 用于：会话/presence、沙箱索引、热点缓存（项目列表）。本期不依赖。

## 5. 多 Project 的 UX 取舍（本期）

- 首页 = 项目列表（空态引导"新建项目"）；工作区页 = 当前三栏 IDE。
- 每个 workspace 只有一条连续 Chat 线程（TSD 已定，不做多线程）。
- 项目可重命名、可删除（软删除先不做，直接删 workspace + messages + 沙箱目录）。
- 移动端：项目列表页同样可用，遵循 AGENTS.md 的 mobile-first。

## 6. 验收标准（Phase 1）

- [ ] 新建项目后，`POST /api/v1/workspaces` 的结果能在 DB 中查到。
- [ ] 重启 API server 后，`GET /api/v1/workspaces` 仍列出项目。
- [ ] 打开旧项目，聊天记录完整恢复（含 tool 消息），可继续对话，上下文正确（compaction 逻辑基于 DB 回放的消息）。
- [ ] 前端项目列表页可创建、进入、（至少）删除项目。
...
- **消息与沙箱漂移**：消息在 DB、文件在 /tmp，重启后可能消息恢复了但文件丢了（例如 /tmp 被清理）。Phase 1 接受这个风险并在项目列表上展示提示；Phase 2 用快照解决。
- **并发**：同一 workspace 的 agent run 已用 `runMu` 串行化；多用户场景未来再考虑行级锁/版本号。
- **ownerID 隔离**：当前所有项目都属于 `anonymous`，接入 Auth 时需要做一次匿名数据迁移（本期不管）。
